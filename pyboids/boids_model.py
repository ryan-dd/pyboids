...
class Boids:
    ...
    def __init__(self):
        ...
        self._boid_velocities = preprocessing.normalize(
            self._boid_velocities, norm='l2')
    ...
    def update_boids(self):
        ...
        all_distances = spatial.distance_matrix(
            self.all_positions, self.all_positions)
        ...
        normalize_new_velocity = preprocessing.normalize(
            new_velocity, norm='l2')
    ...
    def _angle_between(self, vfinal, voriginal):
        ...
        angle = np.arctan2(vfinal[1], vfinal[0]) - \
            np.arctan2(voriginal[1], voriginal[0])
        return angle
    ...
    def _find_ignore_neighbors(self, boid_position, i):
        other_agent_vectors = preprocessing.normalize(
            self.all_positions - boid_position, norm='l2')
        angles = np.dot(other_agent_vectors, self._boid_velocities[i])
        angle_constant = 0.9
        ignore_neighbor_indices = np.where(
            (angles < angle_constant) & (angles > -angle_constant), 1, 0)
        return ignore_neighbor_indices

    def _attraction_rule(self, boid_position, distances):
        attraction_neighbors = np.where(
            (distances > self._zoa_min) & (distances <= self._zoa_max), 1, 0)
        attraction_neighbors_indices = np.where(attraction_neighbors)[0]
        if (len(attraction_neighbors_indices) != 0):
            attraction_neighbors = np.take(
                self.all_positions, attraction_neighbors_indices, axis=0)
    ...
    def _repulsion_rule(self, boid_position, distances, i):
        ...
        if (len(repulsion_neighbors_indices) != 0):
            repulsion_neighbors = np.take(
                self.all_positions, repulsion_neighbors_indices, axis=0)
    ...
    def _orientation_rule(self, boid_position, distances):
        orientation_neighbors = np.where(
            (distances > self._zoo_min) & (distances <= self._zoo_max), 1, 0)
        on_indices = np.where(orientation_neighbors)[0]
        if (len(on_indices) != 0):
            orientation_neighbors = np.take(
                self._boid_velocities, on_indices, axis=0)
